...
本项目是高校《软件测试》课程设计性实验项目，重点展示“多智能体协作”在自动化软件测试中的工程实现。用户可以在前端上传 `.c` / `.h` 文件，启动后端多智能体测试流程，并在页面中查看覆盖率报告、缺陷列表、智能体日志和统计图表。系统既支持命令行运行，也支持 FastAPI + Vue 3 的前后端联动运行方式，便于课堂演示、实验复现和源码审阅。
...
├── CTestAgent/                            # 后端主项目，老师审查智能体实现时重点查看
...
对照实验数据采用项目最终实验记录，评价指标为最终覆盖率、反馈触发次数和缺陷发现数。
...
关键发现：多智能体协作没有降低最终覆盖率和缺陷发现数，同时将反馈触发次数从 2 次提升到 31 次，说明 Reviewer 反馈闭环被有效激活，系统协作过程更充分。
...
| 成员 | 主要负责方向 | 对应模块 |
| --- | --- | --- |
| [姓名 A] | 系统输入端：源码分析与测试用例生成 | Analyzer + Generator |
| [姓名 B] | 系统核心枢纽：执行、审查与调度 | Executor + Reviewer + Coordinator |
| [姓名 C] | 系统输出端：报告、前端与实验文档 | Reporter + 前端 + 实验报告 |

## 12. 文档索引

| 文档 | 作用 |
| --- | --- |
| `README.md` | 项目快速说明、环境配置、启动方式和目录索引。 |
| `设计文档.md` | 最终系统设计文档，完整描述架构、消息协议、协作流程和实验结果。 |
| `三人分工文档.md` | 三位成员的具体分工、负责模块和协作接口。 |
| `设计文档_多智能体协同软件测试系统.md` | 旧版设计文档，保留为新版设计文档的历史基础材料。 |
| `ctestagent-ui/README.md` | Vite 自动生成的前端模板说明，实际项目说明以根目录 README 为准。 |
